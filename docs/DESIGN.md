...
- Researcher Calm results use `Overview` for team-wide evidence and `Participants` for named individual records. Participant mode has no view switcher: it opens directly on the current person's results.
...
- Participant mode leads with the person's name, comparison total, and evaluation records. Aggregate route-choice and reason charts remain available in a collapsed `Response summary` disclosure so they do not compete with the record-review task.
- Names and optional written details are explicitly described as visible to the evaluation team at the point of entry.
- Participant identity uses a generated participant ID that remains stable for the same normalized name on the same device. Session IDs remain separate. Cross-device identity still requires an identity supplied by the production host; analysis must not silently equate sessions with people.
