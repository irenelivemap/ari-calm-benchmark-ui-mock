...
test('provides a deterministic, varied Calm team-results fixture', () => {
  ...
  assert.ok(answers.every(answer => !answer.q2Reasons.includes('more_beautiful_streets_or_surroundings')));
  assert.ok(answers.every(answer => !answer.q2Reasons.includes('familiar_route_or_area')));
  assert.ok(answers.every(answer => !answer.q3Issues.includes('not_enough_beautiful_or_pleasant_surroundings')));
  assert.ok(answers.every(answer => !answer.q3Issues.includes('prefer_another_known_route')));
  assert.ok(answers.some(answer => answer.q3Issues.includes('not_enough_route_near_water')));
