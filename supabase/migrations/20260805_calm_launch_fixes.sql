-- Calm launch integrity fixes.
-- Apply after 20260804_write_only_rpc.sql. This migration does not expose
-- participant data to the browser; anon keeps execute-only access to RPCs.

begin;

create unique index if not exists benchmark_answers_session_round_unique
  on public.benchmark_answers (
    session_id,
    (case when coalesce(payload->>'roundNumber', '') ~ '^[0-9]+$'
      then (payload->>'roundNumber')::integer end)
  )
  where coalesce(payload->>'roundNumber', '') ~ '^[0-9]+$';
  ...
  if test_id = 'calm_route_comparison' then
    ...
    if exists (
      ...
      where value not in (
        ...
        'more_near_water', 'less_need_to_watch_traffic', 'takes_less_time',
      ...
      where value not in (
        ...
        'not_enough_route_near_water', 'too_much_attention_traffic',
...
begin
  if jsonb_typeof(p_record) is distinct from 'object'
    or coalesce(session, '') = ''
    or coalesce(p_record->>'participantName', '') = ''
    or coalesce(test_id, '') not in ('calm_route_comparison', 'calm_vs_fast', 'ari_fast_vs_google')
    or length(session) > 200
    or length(p_record->>'participantName') > 80
    or pg_column_size(p_record) > 65536
  then
  ...
  if saved_at > now() + interval '10 minutes' then
    raise exception 'Progress timestamp is too far in the future' using errcode = '22023';
  end if;

  if test_id = 'calm_route_comparison' then
    if coalesce(p_record->>'source', '') <> 'calm-route-comparison'
      or coalesce(p_record->>'participantId', '') = ''
      or coalesce(p_record->>'roundIndex', '') !~ '^([0-9]|1[0-9]|2[0-2])$'
      or coalesce(p_record->>'completedRounds', '') !~ '^([0-9]|1[0-9]|2[0-3])$'
      or coalesce(p_record->>'questionStep', '') not in ('q1', 'q2', 'q3')
      or jsonb_typeof(p_record->'goalCheckpointPending') is distinct from 'boolean'
    then
      raise exception 'Invalid current Calm progress' using errcode = '22023';
    end if;
    completed := (p_record->>'completedRounds')::integer;
    round_index := (p_record->>'roundIndex')::integer;
    if completed not in (round_index, round_index + 1) then
      raise exception 'Calm progress position is inconsistent' using errcode = '22023';
    end if;
  else
    completed := case when coalesce(p_record->>'completedRounds', '') ~ '^[0-9]+$'
      then (p_record->>'completedRounds')::integer else 0 end;
    round_index := case when coalesce(p_record->>'roundIndex', '') ~ '^[0-9]+$'
      then (p_record->>'roundIndex')::integer else 0 end;
  end if;
  ...
  on conflict (session_id) do update
    ...
    where (case when coalesce(public.benchmark_progress.payload->>'completedRounds', '') ~ '^[0-9]+$'
             then (public.benchmark_progress.payload->>'completedRounds')::integer else 0 end) < completed
       or (
         (case when coalesce(public.benchmark_progress.payload->>'completedRounds', '') ~ '^[0-9]+$'
            then (public.benchmark_progress.payload->>'completedRounds')::integer else 0 end) = completed
         and (case when coalesce(public.benchmark_progress.payload->>'roundIndex', '') ~ '^[0-9]+$'
            then (public.benchmark_progress.payload->>'roundIndex')::integer else 0 end) <= round_index
         and public.benchmark_progress.updated_at <= excluded.updated_at
       );
